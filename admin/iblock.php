...
$sTableID = "tbl_iblock"; // ID таблицы
$oSort = new CAdminSorting($sTableID, "IBLOCK_ID", "asc"); // объект сортировки
...
$sql = "SELECT * FROM `n_multilang_iblock`";
...
while ($arRes = $rsData->NavNext(true, "f_")) {  // создаем строку. результат - экземпляр класса CAdminListRow
	...
	$row->AddViewField("IBLOCK_ID", '<a href="nasledie.multilang_iblock_edit.php?IBLOCK_ID=' . $f_IBLOCK_ID . '&lang=' . LANG . '">' . $f_IBLOCK_ID . '</a>');
	...
	$arActions[] = array(
		...
		"ACTION" => $lAdmin->ActionRedirect("nasledie.multilang_iblock_edit.php?IBLOCK_ID=" . $f_IBLOCK_ID)
	);

	$arActions[] = array(
		"ICON" => "delete",
		"TEXT" => GetMessage("IBLOCK_DEL"),
		"ACTION" => "if(confirm('" . GetMessage('DEL_CONFIRM') . "')) " . $lAdmin->ActionRedirect("nasledie.multilang_iblock_del.php?ID=" . $f_ID)
	);
...
$aContext = array(
	array(
		...
		"LINK" => "nasledie.multilang_iblock_edit.php?lang=" . LANG,
