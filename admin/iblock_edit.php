...
foreach($arLang as $LANG => $L){
	if(isset($NAME[$LANG]) && $IBLOCK_ID>0){
	...
	$DB->StartTransaction();
	if ($ID[$LANG] > 0) {
		$DB->Update("n_multilang_iblock", $arFields, "WHERE ID='" . $ID[$LANG] . "'", $err_mess . __LINE__);
	} else {
		$DB->Insert("n_multilang_iblock", $arFields, $err_mess . __LINE__);
	}
	$DB->Commit();
	}
}
	...
	if ($IBLOCK_ID > 0) {
		...
		if ($apply != "") {
			...
			LocalRedirect("/bitrix/admin/nasledie.multilang_iblock_edit.php?IBLOCK_ID=" . $IBLOCK_ID . "&mess=ok&lang=" . LANG . "&" . $tabControl->ActiveTabParam());
		} else {
...
if ($IBLOCK_ID > 0) {
	$sql = "SELECT * FROM `n_multilang_iblock` WHERE `IBLOCK_ID`='" . $DB->ForSql($IBLOCK_ID) . "'";
	$rsData = $DB->Query($sql, false, __FILE__ . " > " . __LINE__);
	while ($row = $rsData->Fetch()) {
		$str_ID[$row['LANG']] = $row['ID'];
		$str_IBLOCK_ID[$row['LANG']] = $row['IBLOCK_ID'];
				...
				<div class="adm-detail-content" style="padding: 12px 18px 12px 12px;">
					<div class="adm-detail-content-item-block">
						<table class="adm-detail-content-table edit-table">
							<tr>
								<td><span class="required">*</span>IBLOCK</td>
								<td>
									<select name="IBLOCK_ID">
										<? foreach ($arIB as $type => $list) { ?>
											<optgroup label="<?= $type ?>">
												<? foreach ($list as &$IB) { ?>
...
// отобразим заголовки закладок
	...
	$tabControl->Buttons(
		array(
			"back_url" => "nasledie.multilang_iblock.php?lang=" . LANG,
		)
	);
	$tabControl->End();
	?>
