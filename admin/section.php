...
$sTableID = "tbl_section"; // ID таблицы
$oSort = new CAdminSorting($sTableID, "SECTION_ID", "asc"); // объект сортировки
...
$sql = "SELECT * FROM `n_multilang_section`";
...
$lAdmin->AddHeaders(array(
	...
	array("id" => "SECTION_ID",
		"content" => GetMessage("TBL_SECTION"),
		"sort" => "SECTION_ID",
		"default" => true,
	),
	array("id" => "NAME",
...
while ($arRes = $rsData->NavNext(true, "f_")) {  // создаем строку. результат - экземпляр класса CAdminListRow
	...
	$row->AddViewField("SECTION_ID", '<a href="nasledie.multilang_section_edit.php?SECTION_ID=' . $f_SECTION_ID . '&lang=' . LANG . '">' . $f_SECTION_ID . '</a>');
	...
	$arActions[] = array(
		...
		"ACTION" => $lAdmin->ActionRedirect("nasledie.multilang_section_edit.php?SECTION_ID=" . $f_SECTION_ID)
	);

	$arActions[] = array(
		"ICON" => "delete",
		"TEXT" => GetMessage("IBLOCK_DEL"),
		"ACTION" => "if(confirm('" . GetMessage('DEL_CONFIRM') . "')) " . $lAdmin->ActionRedirect("nasledie.multilang_section_del.php?ID=" . $f_ID)
	);
...
$aContext = array(
	array(
		...
		"LINK" => "nasledie.multilang_section_edit.php?lang=" . LANG,
...
<form name="find_form" method="get" action="<? echo $APPLICATION->GetCurPage(); ?>">
	...
	<tr>
		<td>SECTION_ID:</td>
		<td><input type="text" name="find_section"  value="<? echo htmlspecialchars($find_section) ?>"></td>
	</tr>
